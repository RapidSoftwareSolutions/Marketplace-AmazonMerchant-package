...
$app->post('/api/AmazonMerchant/createShipment', function ($request, $response, $args) {
    
    $settings =  $this->settings;
    
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['apiKey','apiSecret','appName','appVersion','region','merchantId','amazonOrderId','itemList','shipFromAddress','packageDimensions','weight','shippingServiceOptions','shippingServiceId']);
    ...
    $req = new MWSMerchantFulfillmentService_Model_CreateShipmentRequest();
    ...
    $req->setShippingServiceId($post_data['args']['shippingServiceId']);
    
    if(!empty($post_data['args']['shippingServiceOfferId'])) {
        $req->setShippingServiceOfferId($post_data['args']['shippingServiceOfferId']);
    }
    if(!empty($post_data['args']['hazmatType'])) {
        $req->setHazmatType($post_data['args']['hazmatType']);
    }
    
    
    try {
        $xml = simplexml_load_string($service->CreateShipment($req)->toXML());
