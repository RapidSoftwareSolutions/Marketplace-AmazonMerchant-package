...
$app->post('/api/AmazonMerchant/getShipment', function ($request, $response, $args) {
    ...
    $req = new MWSMerchantFulfillmentService_Model_GetShipmentRequest();
    $req->setSellerId($post_data['args']['merchantId']);
    $req->setShipmentId($post_data['args']['shipmentId']);
    
    try {
        $xml = simplexml_load_string($service->GetShipment($req)->toXML());
